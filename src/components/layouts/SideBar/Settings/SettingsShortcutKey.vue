    ...
    <a-form :model="formState" :label-col="labelCol">
      <a-card title="老板键" size="small">
        <a-form-item label="快捷键">
          <a-input
            v-model:value="formState.lock.key"
            readonly
        ...
        <a-form-item label="打开网址" class="bottom-0">
          <a-select
            v-model:value="formState.lock.action"
            :options="formState.lock.actionList.map((item) => ({ value: item }))"
          >
      ...
      <a-card title="随机背景" size="small" style="margin-top: 12px">
        <a-form-item label="快捷键" class="bottom-0">
          <a-input
            v-model:value="formState.wallpaper.key"
            readonly
            :placeholder="`例如：${useSystemStore.ctrlOrCommand()} + E`"
          />
        </a-form-item>
      </a-card>
    </a-form>
