<template>
  <button
    ...
    type="button"
    role="switch"
    :title="`切换为${useSystemStore.darkMode ? '日间' : '夜间'}模式`"
    aria-checked="false"
    :class="{dark: useSystemStore.darkMode}"
    @click="changeMode"
  >
    <span class="check" data-v-b171b020="">
      <span class="icon" data-v-b171b020="">
        <svg
...
watch(() => useSystemStore.darkMode, (newVal) => {
  if (newVal) {
...
const changeMode = () => {
  ...
  let isDark = useSystemStore.darkMode;
  if (!isDark) {
    // html添加theme 属性值
